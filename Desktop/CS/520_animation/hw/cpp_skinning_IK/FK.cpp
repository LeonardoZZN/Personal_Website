    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // Also useful are the Mat3d and RigidTransform4d classes defined in the Vega folder.
   ...
   Mat3d rot; // rotational part in localTransform
   double orientRot[9], localRot[9]; // rotation of the joint's frame to the joint's parent frame; rotation in the joint's local frame

   for (int jointID : jointUpdateOrder) {
      euler2Rotation(jointOrientationEulerAngles[jointID], orientRot, RotateOrder::XYZ);
      euler2Rotation(eulerAngles[jointID], localRot, rotateOrders[jointID]);
      rot = asMat3d(orientRot) * asMat3d(localRot);
      // Construct the local transformation matrix of the current joint
      RigidTransform4d localTrans(rot, translations[jointID]);
      localTransforms[jointID] = localTrans;

      // Compute the global transformation matrix of the current joint
      if (jointParents[jointID] < 0) {
         globalTransforms[jointID] = localTransforms[jointID];
      }
      else {
         globalTransforms[jointID] = globalTransforms[jointParents[jointID]] * localTransforms[jointID];
      }
   }
}
