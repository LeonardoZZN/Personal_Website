...
static IKMethod IKAlgorithm = Tikhonov;
static SkinMethod skinMethod = LBS;
// for screenshots
int sprite = 0; // number of images saved to disk so far
int saveScreenToFile = 0; // trigger of the image saving mechanism 
//======================= Functions =============================
...
static void idleFunction()
{
  ...
  if (elapsedTime >= 1.0 / 4)
  {
    ...
    const char* skinMethodName;
    if (skinMethod == LBS) {
       skinMethodName = "Linear Blend Skinning";
    }
    else {
       skinMethodName = "Dual Quaternion Skinning";
    }
    sprintf(windowTitle, "Vertices: %d | %.1f FPS | graphicsFrame %d | IK Method: %s | Skinning Method: %s",
       meshDeformable->Getn(), fpsBuffer.getAverage(), graphicsFrameID, IKAlgName, skinMethodName);
...
static void initialize()
{
  ...
  skinning->setSkinMethod(skinMethod); 
...
int main (int argc, char ** argv)
{
  ...
  if ( argc < numFixedArgs )
  {
    ...
    cout << "Usage: " << argv[0] << " <configFilename>" << " <IKMethod>" << " <SkinningMethod>" << endl;
    cout << "IKMethod:" << endl;
    cout << "   t: Tikhonov Regularization Method" << endl;
    cout << "   p: Pseudo-Inverse Method" << endl;
    cout << "SkinningMethod:" << endl;
    cout << "   l: Linear Blend Skinning (LBS)" << endl;
    cout << "   d: Dual-Quaternion Skinning (DQS)" << endl;
    cout << "Example: " << argv[0] << " skin.config" << " t" << " l" << endl;
    return 0;
  }
 
  configFilename = argv[1];
  char* IKMethodName = argv[2];
  if (IKMethodName[0] == 't') {
     IKAlgorithm = Tikhonov;
  }
  else if (IKMethodName[0] == 'p') {
     IKAlgorithm = Pseudoinverse;
  }
  else {
     cout << "Input Error: Invalid IK Method" << endl;
     cout << "Available IK Method: Tikhonov(t), Pseudo-Inverse(p)" << endl;
     return 0;
  }
  char* skinMethodName = argv[3];
  if (skinMethodName[0] == 'l') {
     skinMethod = LBS;
  }
  else if (skinMethodName[0] == 'd') {
     skinMethod = DQS; 
  }
  else {
     cout << "Input Error: Invalid Skinning Method" << endl;
     cout << "Available Skinning Method: Linear Blend Skinning(LBS), Dual Quaternion Skinning(DQS)" << endl;
     return 0;
  }
  initConfigurations();
